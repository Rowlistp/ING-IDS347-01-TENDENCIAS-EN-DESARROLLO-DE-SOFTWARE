...
- ~~Mejora aprobada por el equipo (Builder 1 y Builder 3): el combustible pertenece al vehículo y el empleado puede tener un vehículo habitual.~~ **IMPLEMENTADO:** `Vehiculo.TipoCombustibleId` (obligatorio al crear; nulo solo en vehículos previos, que se corrigen al editarlos) y `Empleado.VehiculoHabitualId` (opcional). La solicitud propone ambos valores por defecto y sigue siendo editable. Pendiente para decidir: si la API de solicitudes debe rechazar un combustible distinto al del vehículo (hoy el frontend solo avisa).
...
- ~~Recepciones — el comprobante no era un PDF real: "Descargar comprobante" generaba un HTML autocontenido en el navegador, a diferencia de Tickets y Cierre Diario, que sí tienen un PDF generado en el servidor.~~ **RESUELTO:** se agregó `GET /api/v1/recepciones/{id}/pdf`, con `RecepcionPdfService` (QuestPDF, mismo estilo visual que `CierreDiarioService`) y el campo `RecepcionCombustible.PdfComprobante` como caché, igual patrón que `CierreDiario.PdfActa`. El botón "Descargar comprobante" de `RecepcionesPage.jsx` ahora usa `apiDownload` contra ese endpoint, igual que Tickets y Cierre Diario; se eliminó `descargarComprobanteHtml()` de `utils/download.js` por quedar sin uso. El botón "Imprimir comprobante" (impresión en el navegador) se mantuvo sin cambios. Verificado en vivo: la descarga produce un archivo con firma `%PDF-` real.
