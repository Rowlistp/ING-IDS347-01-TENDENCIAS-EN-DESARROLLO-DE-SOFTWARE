...
- Asimetría de validación en Recepciones: el backend valida TANQUE_INACTIVO al registrar una recepción, pero no valida que el Proveedor esté activo. Es posible registrar una recepción con un proveedor desactivado. No es bloqueante, pero es inconsistente con el patrón de validación usado en el resto del sistema. Pendiente de decisión del equipo (Builder 1): agregar la validación PROVEEDOR_INACTIVO si se considera necesario.
- Tanques y Vehículos no tienen forma de reactivarse una vez desactivados: `SaveTanqueRequest` y `SaveVehiculoRequest` no incluyen el campo `Activo`, a diferencia de `SaveDepartamentoRequest`, `SaveEmpleadoRequest`, `SaveProveedorRequest` y `SaveTipoCombustibleRequest`, que sí lo tienen (y cuyo frontend ya expone un checkbox "Activo" en el formulario de edición para reactivar). Un tanque o vehículo desactivado queda inactivo permanentemente sin intervención directa en la base de datos. Pendiente de decisión del equipo (Builder 1): agregar `Activo` a ambos DTOs si se considera necesario.
- Estaciones — asimetría de lectura: `GET /api/v1/estaciones` solo devuelve estaciones activas (`.Where(e => e.Activo)`), a diferencia de Tanques/Proveedores/TiposCombustible/Departamentos/Empleados, que devuelven todas (activas e inactivas) para su gestión administrativa. El frontend lo resolvió manteniendo la fila desactivada en estado local en vez de recargar la lista completa, pero es una inconsistencia real del backend frente al patrón usado en el resto de catálogos.
- Estaciones — asimetría de validación: `PUT /api/v1/estaciones/{id}` puede desactivar una estación (`activo: false`) sin ninguna validación de dependencias, mientras que `DELETE /api/v1/estaciones/{id}` sí valida `ESTACION_CON_DESPACHOS` antes de desactivar. El formulario de edición del frontend nunca envía `activo: false` (evita este hueco desde la UI), pero el backend permitiría desactivar una estación con despachos asociados si se llamara `PUT` directamente con ese valor.
